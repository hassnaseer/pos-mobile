...
const SupportDashboardScreen = () => {
  ...
  useEffect(() => {
    ...
    const unsub = db.collection('support_conversations')
      ...
      .onSnapshot(snap => {
        ...
        // support-staff sees unassigned + assigned to them
        const filtered = all.filter(c => c.assignedToId === null || c.assignedToId === user.id);
  ...
  const claimConversation = async convId => {
    if (!firestore) return;
    const db = firestore();
  ...
  const send = async () => {
    ...
    if (!conv) return;
    // Support staff can only reply if assigned to them or unassigned
    const canReply = conv.assignedToId === null || conv.assignedToId === user.id;
    if (!canReply) return;
    ...
    await db.collection('support_conversations').doc(selectedId).collection('messages').add({
      senderId: user.id, senderName: user.name, senderRole: 'support-staff',
  ...
  return (
    <View style={styles.root}>
      ...
      <View style={styles.convList}>
        <Text style={styles.convTitle}>Support Queue</Text>
            ...
            renderItem={({ item }) => (
              ...
              >
                ...
                <View style={styles.convInfo}>
                  ...
                  <Text style={styles.convSub} numberOfLines={1}>{item.assignedToId ? `Assigned: ${item.assignedToName}` : 'Unassigned'}</Text>
                </View>
                {item.isReadBySupport === false && <View style={styles.unreadDot} />}
              </TouchableOpacity>
            )}
            ListEmptyComponent={<Text style={styles.noConvs}>No conversations in queue.</Text>}
          />
        )}
      </View>
          ...
          <>
            <View style={styles.chatHeader}>
              ...
              {selected.assignedToId !== user.id && (
                <TouchableOpacity style={styles.claimBtn} onPress={() => claimConversation(selected.id)}>
            ...
            {(selected.assignedToId === null || selected.assignedToId === user.id) && (
            ...
            {selected.assignedToId && selected.assignedToId !== user.id && (
              <View style={styles.assignedBanner}>
                <Text style={styles.assignedText}>Assigned to {selected.assignedToName}</Text>
              </View>
            )}
          </>
        )}
...
const styles = StyleSheet.create({
  ...
  convSub: { fontSize: 10, fontFamily: 'Outfit-Regular', color: colors.secondary, marginTop: 2 },
  unreadDot: { width: 8, height: 8, borderRadius: 4, backgroundColor: colors.primary },
  noConvs: { textAlign: 'center', color: colors.secondary, fontFamily: 'Outfit-Regular', padding: 20 },
  chatPanel: { flex: 1 },
  chatHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', padding: 12, backgroundColor: '#fff', borderBottomWidth: 1, borderColor: '#eee' },
  ...
  claimBtn: { backgroundColor: colors.primary, borderRadius: 8, paddingHorizontal: 14, paddingVertical: 7 },
  claimBtnText: { color: '#fff', fontFamily: 'Outfit-SemiBold', fontSize: 13 },
  ...
  emptyChat: { textAlign: 'center', color: colors.secondary, fontFamily: 'Outfit-Regular', marginTop: 40 },
  ...
  assignedBanner: { padding: 12, backgroundColor: '#f9fafb', borderTopWidth: 1, borderColor: '#eee', alignItems: 'center' },
  assignedText: { fontSize: 13, fontFamily: 'Outfit-Regular', color: colors.secondary },
});
