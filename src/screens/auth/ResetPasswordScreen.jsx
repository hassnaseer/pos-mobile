...
const ResetPasswordScreen = ({ navigation, route }) => {
  ...
  const handleSubmit = async ({ newPassword }) => {
    try {
      await mutateAsync({ email, otp: otpCode, newPassword });
      Alert.alert('Success', 'Your password has been reset. Please log in.', [
        { text: 'OK', onPress: () => navigation.navigate('Login') },
      ]);
    } catch (err) {
      Alert.alert('Error', typeof err === 'string' ? err : err?.message ?? 'Failed to reset password.');
    }
  };
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        <View style={styles.card}>
          ...
          <View style={styles.emailBadge}>
            <Text style={styles.emailBadgeText}>🔒  Resetting for: <Text style={styles.emailBold}>{email}</Text></Text>
          </View>
...
const styles = StyleSheet.create({
  ...
  card:          { backgroundColor: '#fff', borderRadius: 16, padding: 24, alignItems: 'center', shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.08, shadowRadius: 8, elevation: 4 },
  icon:          { fontSize: 36, marginBottom: 12 },
  title:         { fontSize: 26, fontFamily: 'Outfit-Bold', color: colors.defaultBlack, marginBottom: 8 },
  subtitle:      { fontSize: 14, fontFamily: 'Outfit-Regular', color: colors.secondary, marginBottom: 16, lineHeight: 22, textAlign: 'center' },
  emailBadge:    { backgroundColor: '#EFF6FF', borderRadius: 10, paddingHorizontal: 14, paddingVertical: 10, marginBottom: 24, width: '100%' },
  ...
  input:         { borderWidth: 1.5, borderColor: '#D0D5DD', borderRadius: 8, paddingHorizontal: 14, paddingVertical: 12, fontSize: 15, fontFamily: 'Outfit-Regular', color: colors.text, width: '100%' },
  inputErr:      { borderColor: colors.warning },
  errText:       { color: colors.warning, fontSize: 12, fontFamily: 'Outfit-Regular', marginTop: 4 },
  showPwWrap:    { alignItems: 'flex-end', marginBottom: 8, width: '100%' },
  showPwText:    { color: colors.primary, fontSize: 13, fontFamily: 'Outfit-Regular' },
  btn:           { backgroundColor: colors.primary, borderRadius: 8, paddingVertical: 14, alignItems: 'center', marginTop: 4, width: '100%' },
  btnText:       { color: '#fff', fontSize: 16, fontFamily: 'Outfit-SemiBold' },
  backWrap:      { marginTop: 20 },
